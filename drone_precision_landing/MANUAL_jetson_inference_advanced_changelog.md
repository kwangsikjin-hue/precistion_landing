...
| `adab794` | fix | 1건 | `cv2.aruco` 미설치 시 크래시 방지 — contrib 설치 안내 및 자동 비활성화 |
| `026838a` | fix | 2건 | RealSense 버퍼 공유 잔상 수정 (`.copy()`) + `--motp-log on/off` 추가 |
| `02ec19e` | fix | 3건 | ByteTrack 중복 영상 3가지 근본 원인 수정 |
| `d70f511` | fix | 4건 | 전체 정밀 분석 — ArUco az 방어·track_color 캐싱·TRT 주석 등 |
| `4676e6f` | fix | 3건 | NMS 추가로 바운딩박스 중복 근본 해결 + 텍스트 위치 조정 |
| `9b6273f` | fix | 1건 | `--motp-log off` 시 처음부터 `_log` 누적 자체 차단 |
...
### 커밋 `adab794` — `cv2.aruco` 미설치 크래시 방지 (fix)

**에러:** `AttributeError: module 'cv2' has no attribute 'aruco'`

```python
# 수정 전: 딕셔너리가 모듈 로딩 시 cv2.aruco 즉시 참조 → 크래시
_ARUCO_DICT_MAP = {'4X4_50': cv2.aruco.DICT_4X4_50, ...}  # ← 모듈 로딩 시 실패

# 수정 후: 가용 여부 확인 후 조건부 참조
_ARUCO_AVAILABLE = hasattr(cv2, 'aruco')
if args.aruco == 'on' and not _ARUCO_AVAILABLE:
    print("⚠️ cv2.aruco 모듈 없음 — pip3 install opencv-contrib-python")
    _aruco_detect = None   # 자동 비활성화
elif args.aruco == 'on' and _ARUCO_AVAILABLE:
    _ARUCO_DICT_MAP = {'4X4_50': cv2.aruco.DICT_4X4_50, ...}  # ← 안전
```

---

### 커밋 `026838a` — 잔상·중복 영상 + `--motp-log` (fix)

#### 잔상 원인 및 수정

```python
# 수정 전: View 반환 → 그리기가 RealSense 버퍼 오염 → 다음 프레임에 잔상
color_image = np.asanyarray(color_frame.get_data())

# 수정 후: 독립 복사본 → 그려도 버퍼 오염 없음
color_image = np.asanyarray(color_frame.get_data()).copy()
```

#### `--motp-log on/off` 추가 (1차 구현)

종료 시 CSV 저장 여부 제어. 이후 `9b6273f`에서 더 강화됨.

---

### 커밋 `02ec19e` — ByteTrack 중복 영상 3가지 원인 수정 (fix)

#### 원인 1: YOLO가 ArUco 그림 위에서 추론

```python
# 수정 전: ArUco 그리기 → YOLO 추론 (YOLO가 그림을 착륙 패드로 오탐)
# 수정 후: YOLO 추론 → ArUco 그리기 (깨끗한 이미지로 추론)
raw_out = trt_brain.infer(color_image)   # ← YOLO 먼저
...
cv2.aruco.drawDetectedMarkers(...)        # ← ArUco 그리기는 나중에
```

#### 원인 2: `draw_info`가 모든 트랙에 호출되어 텍스트 중복

```python
# 수정 전: for loop 내에서 트랙마다 draw_info 호출 → 텍스트 N개 겹침
# 수정 후: for loop에서는 바운딩박스+ID만, draw_info는 primary 블록에서 1회만
```

#### 원인 3: ArUco ON 시 ByteTrack Kalman 필터 이중 업데이트

```python
# 수정 전: for loop update() + primary 블록 update() → 같은 KF 2회 업데이트
# 수정 후: primary 블록에서는 각도만 교체, update() 재호출 없음
angle_x = math.atan2(ax, safe_az)   # KF update() 없이 각도만 교체
```

---

### 커밋 `d70f511` — 전체 정밀 분석 4건 수정 (fix)

| 항목 | 수정 내용 |
|------|-----------|
| ArUco `az≈0` 방어 | `az < DEPTH_MIN_M` 이면 ArUco 무효 → YOLO 폴백, `safe_fz = max(fz, 1e-4)` |
| `track_color()` 캐싱 | `np.random.RandomState` 매 프레임 생성 → `_color_cache` 딕셔너리로 캐싱 |
| ByteTrack ArUco `az` 검증 | `az >= DEPTH_MIN_M` + `safe_az = max(az, 1e-4)` 추가 |
| TRT 버퍼 View 경고 주석 | `infer()` 반환값이 내부 버퍼 View임을 주석으로 명시 |

---

### 커밋 `4676e6f` — 바운딩박스 중복 근본 원인 수정 (fix)

#### NMS — 핵심 수정

```python
# 수정 전: YOLO 8400개 → 임계값 필터 → ByteTracker (중복 박스 다수 입력 → 다중 트랙)
# 수정 후: YOLO 8400개 → NMS → ByteTracker (중복 제거 후 입력)
...
```
수정 전: y=20(ByteTrack 헤더) / y=25(draw_info) → 5px 차이, 겹침
수정 후: y=20(ByteTrack 헤더) / y=75(draw_info, y_top=130) → 55px 간격
```

#### ByteTracker 파라미터 조정

```python
ByteTracker(match_thresh=0.7, max_lost=15)  # 0.8→0.7, 30→15
```

---

### 커밋 `9b6273f` — `--motp-log off` 처음부터 스킵 (fix)

```python
# 수정 전: --motp-log off 여도 매 프레임 _log.append() 실행, 마지막에만 저장 스킵
# 수정 후: save_log 플래그로 처음부터 누적 자체 차단
class MOTPEvaluator:
    def __init__(self, save_log: bool = True):
        self._log = [] if save_log else None   # off 이면 None으로 미생성

    def update(self, innov):
        if self._save_log:
            self._log.append(...)   # off 이면 이 줄 실행 안 됨
```

---

### 커밋 `3c6c901` — `TrajectoryLogger` + `--traj-log` (feat)

```python
# 모든 로그 on/off 패턴 통일:
# --motp-log off → MOTPEvaluator(save_log=False)  → _log=None, 처음부터 스킵
# --traj-log off → TrajectoryLogger(save_log=False) → _log=None, 처음부터 스킵
# --predict 0   → lstm_pred=None                   → 생성 자체 없음
```

**저장 파일:** `trajectory_log.csv`  
**컬럼:** `time_s, source, track_id, x_m, y_m, z_m, vx, vy, vz, speed_mps`  
**최대 항목:** 54,000건 (30fps × 30분), 초과 시 앞 절반 자동 삭제

---

### 커밋 `76d05ef` — ByteTrack `draw_info` y 위치 조정 (fix)

```
수정 전: y_top=130 → 텍스트 y=75,90,105,120 (헤더에서 55px 간격, 너무 내려감)
수정 후: y_top=93  → 텍스트 y=38,53,68,83  (헤더에서 18px 간격, 적정)

레이아웃:
  y= 20: ByteTrack | CTRV | active:1  (헤더)
  y= 38: Offset X: ...
  y= 53: Alt(Z): ...
  y= 68: Vel Vx: ...
  y= 83: Speed: ...
...
### 커밋 `c579a1a` — 정밀 재분석 5건 수정 (fix)

| 항목 | 코드 수정 |
|------|----------|
| **E1** ByteTrack LSTM 트랙 혼입 | `_lstm_primary_id`로 주 트랙 변경 감지 → `lstm_pred.reset()` |
| **E2** Single YOLO draw_info y 충돌 | `draw_info(x1, max(y1,110), ...)` — y_top 최소 110 보장 |
| **E3** `_color_cache` 영구 누적 | `track_3d_kfs` 정리 시 `_color_cache.pop(tid)` 함께 제거 |
| **E4** ByteTrack `angle_x/y` scope | `primary` 튜플에서 가져옴을 주석으로 명시 |
| **E5** ArUco 추적 원 미표시 | `cv2.circle(cx_a, cy_a, 7, (0,255,255))` 추가 |

```python
# E1: LSTM 트랙 혼입 방지
if ptid != _lstm_primary_id:
    lstm_pred.reset()        # 다른 트랙 궤적 데이터 제거
    _lstm_primary_id = ptid  # 새 주 트랙 ID 기록

# E3: 색상 캐시 함께 정리
for tid in list(track_3d_kfs):
    if tid not in active_ids:
        track_3d_kfs[tid].reset(); del track_3d_kfs[tid]
        _color_cache.pop(tid, None)   # 추가
```
...
| 한글→ASCII | `2213c23` | `CTRV[직선/선회]` → `CTRV[Line/Turn]`, `활성` → `active` |
| 소스 표시 | `9265db9` | `SRC:ArUco`, `SRC:YOLO+D` 구분 출력 |
| ArUco 추적 원 | `c579a1a` | ArUco 경로에 `cv2.circle()` 추가 (YOLO 경로와 일관성) |
| angle scope | `c579a1a` | primary 튜플 angle 사용 명시 주석 |
...
| `TC` | `d70f511` | `track_color()` `_color_cache` 캐싱 (매 프레임 RandomState 제거) |
| `NMS` | `4676e6f` | `cv2.dnn.NMSBoxes` 적용 → ByteTracker 중복 트랙 제거 |
...
| `A7` | `adab794` | `cv2.aruco` 미설치 자동 감지 → 설치 안내 + 자동 비활성화 |

### 🔶 영상 중복·잔상 수정 (8건)

| 항목 | 커밋 | 내용 |
|------|------|------|
| `V1` | `026838a` | `np.asanyarray().copy()` — RealSense 버퍼 공유 잔상 제거 |
| `V2` | `02ec19e` | YOLO 추론 순서 수정 (ArUco 그리기 이후 → 이전으로) |
| `V3` | `02ec19e` | ByteTrack `draw_info` for 루프에서 제거, primary 1회만 호출 |
| `V4` | `02ec19e` | ArUco ON 시 ByteTrack KF 이중 업데이트 방지 |
| `V5` | `4676e6f` | NMS 적용 → ByteTracker 중복 박스 입력 차단 |
| `V6` | `4676e6f` | 좌측상단 텍스트 y좌표 충돌 수정 (y_top 80→130) |
| `V7` | `76d05ef` | ByteTrack draw_info y 위치 재조정 (y_top 130→93, 너무 낮음 수정) |
| `V8` | `c579a1a` | Single YOLO draw_info y 하한 보장 `max(y1,110)` |

### 🟤 로그 on/off 처음부터 스킵 패턴 (4건)

| 항목 | 커밋 | 내용 |
|------|------|------|
| `L1` | `026838a` | `--motp-log on/off` 인수 추가 (1차) |
| `L2` | `9b6273f` | `--motp-log off` 시 `_log=None` 처음부터 스킵 (패턴 완성) |
...
| 항목 | 내용 |
|------|------|
| E1 | ByteTrack LSTM 버퍼 주 트랙 ID 변경 시 리셋 미실행 |
| E2 | Single YOLO `draw_info(x1,y1)` SRC 텍스트(y=20)와 충돌 |
| E3 | `_color_cache` 소멸 트랙 미정리 → 장시간 누적 |
| E4 | ByteTrack 루프 후 `angle_x/y` scope 혼용 가독성 |
| E5 | ArUco 경로 추적 원(`cv2.circle`) 미표시 |
...
| `--motp-log` | `on` | MOTP CSV 저장 (`on`=저장, `off`=처음부터 누적 안 함) |
| `--traj-log` | `off` | 궤적 CSV 저장 (`on`=저장, `off`=처음부터 누적 안 함) |
...
9b6273f fix: --motp-log off 시 처음부터 _log 누적 자체 차단
4676e6f fix: 바운딩박스 중복·좌측상단 텍스트 겹침 근본 원인 수정
d70f511 fix: 전체 정밀 분석 결과 4건 수정
02ec19e fix: ByteTrack 중복 영상 3가지 원인 수정
026838a fix: RealSense 버퍼 공유로 인한 잔상·중복 영상 수정 + --motp-log on/off 추가
adab794 fix: cv2.aruco 미설치 시 크래시 방지 — contrib 패키지 안내 및 자동 비활성화
